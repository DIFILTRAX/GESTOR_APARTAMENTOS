...
import { SeguridadService, Rol } from '../../../core/services/seguridad.service';
  ...
  template: `
    <div class="page">
      <section class="head">
        ...
        <button class="btn btn-primary" (click)="abrirCrear()">+ Nuevo rol</button>
      </section>
      ...
      <div class="table-wrap">
        <table class="tabla">
          ...
          <tbody>
            @for (r of filtrados(); track r.id_rol) {
              <tr>
                <td class="mono">{{ r.id_rol }}</td>
                <td class="bold">{{ r.nombre }}</td>
                <td>
                  <div class="actions">
                    <button class="btn btn-sm btn-ghost" (click)="abrirEditar(r)">Editar</button>
                    <button class="btn btn-sm btn-danger" (click)="confirmarEliminar(r)">Eliminar</button>
                  </div>
                </td>
              </tr>
            }
        ...
        <div class="backdrop" (click)="cerrarModal()">
          <div class="modal" (click)="$event.stopPropagation()">
            ...
            <div class="modal-body">
              <div class="field">
                <label>Nombre *</label>
                <input [(ngModel)]="form.nombre" placeholder="Ej: Administrador" />
              </div>
            </div>
      ...
      @if (eliminarTarget()) {
        <div class="backdrop" (click)="eliminarTarget.set(null)">
          <div class="modal modal-sm" (click)="$event.stopPropagation()">
            ...
            <div class="modal-body">
              <p>¿Eliminar el rol <strong>{{ eliminarTarget()!.nombre }}</strong>?</p>
...
export class RolesComponent implements OnInit {
  ...
  eliminarTarget = signal<Rol | null>(null);
  form: Partial<Rol> = {};

  filtrados = computed(() => {
    const q = this.busqueda().toLowerCase();
    return this.roles().filter(r => !q || r.nombre.toLowerCase().includes(q));
  });
  ...
  guardar(): void {
    ...
    if (this.modoEditar()) {
      this.svc.actualizarRol(this.form.id_rol!, this.form).subscribe({
        next: () => { this.toast.success('Rol actualizado.'); this.cerrarModal(); this.guardando.set(false); },
        error: () => { this.toast.error('Error al actualizar.'); this.guardando.set(false); }
      });
    } else {
      this.svc.crearRol(this.form).subscribe({
        next: () => { this.toast.success('Rol creado.'); this.cerrarModal(); this.guardando.set(false); },
        error: () => { this.toast.error('Error al crear.'); this.guardando.set(false); }
      });
    }
  }

  confirmarEliminar(r: Rol): void { this.eliminarTarget.set(r); }

  eliminar(): void {
    const target = this.eliminarTarget();
    if (!target) return;
    this.guardando.set(true);
    this.svc.eliminarRol(target.id_rol).subscribe({
      next: () => { this.toast.success('Rol eliminado.'); this.eliminarTarget.set(null); this.guardando.set(false); },
      error: () => { this.toast.error('Error al eliminar. Puede tener perfiles asociados.'); this.guardando.set(false); }
    });
  }
}
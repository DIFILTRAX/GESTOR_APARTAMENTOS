...
import { SeguridadService, Perfil } from '../../../core/services/seguridad.service';
...
@Component({
  ...
  template: `
    <div class="page">
      <section class="head">
        ...
        <button class="btn btn-primary" (click)="abrirCrear()">+ Nuevo perfil</button>
      </section>

      <div class="toolbar">
        <input class="search" [(ngModel)]="busqueda" placeholder="Buscar por nombre o rol..." />
      ...
      <div class="table-wrap">
        <table class="tabla">
          ...
          <tbody>
            @for (p of filtrados(); track p.id_perfil) {
              <tr>
                <td class="mono">{{ p.id_perfil }}</td>
                <td class="bold">{{ p.nombre }}</td>
                <td>
                  <span class="badge badge-blue">{{ p.rol_nombre ?? p.rol }}</span>
                </td>
                <td>
                  <div class="actions">
            ...
            @empty {
              <tr><td colspan="4" class="empty">No hay perfiles registrados.</td></tr>
            }
          </tbody>
        </table>
      </div>
        ...
        <div class="backdrop" (click)="cerrarModal()">
          <div class="modal" (click)="$event.stopPropagation()">
            <div class="modal-head">
              <h3>{{ modoEditar() ? 'Editar perfil' : 'Nuevo perfil' }}</h3>
              <button class="close" (click)="cerrarModal()">✕</button>
            </div>
            <div class="modal-body">
              <div class="field">
                <label>ID Perfil *</label>
                <input [(ngModel)]="form.id_perfil"
                       type="number" min="1"
                       [disabled]="modoEditar()"
                       placeholder="Ej: 3" />
              ...
              <div class="field">
                <label>Nombre *</label>
                <input [(ngModel)]="form.nombre" placeholder="Ej: Supervisor" />
              </div>
              <div class="field">
                <label>Rol *</label>
                <select [(ngModel)]="form.rol">
                  <option [ngValue]="undefined">Seleccionar...</option>
                  @for (r of svc.roles(); track r.id_rol) {
      ...
      @if (eliminarTarget()) {
        <div class="backdrop" (click)="eliminarTarget.set(null)">
          <div class="modal modal-sm" (click)="$event.stopPropagation()">
            ...
            <div class="modal-body">
              <p>¿Eliminar el perfil <strong>{{ eliminarTarget()!.nombre }}</strong>?</p>
  ...
  styles: [`
    ...
    .modal { background: #fff; border-radius: 14px; width: min(460px, 100%); box-shadow: 0 24px 60px rgba(0,0,0,.2); display: grid; overflow: hidden; }
...
export class PerfilesComponent implements OnInit {
  ...
  perfiles       = this.svc.perfiles;
  busqueda       = signal('');
  modalAbierto   = signal(false);
  modoEditar     = signal(false);
  guardando      = signal(false);
  eliminarTarget = signal<Perfil | null>(null);
  form: Partial<Perfil> = {};
  ...
  ngOnInit(): void {
    if (this.svc.perfiles().length === 0) {
      this.svc.cargarPerfiles().subscribe({ error: () => this.toast.error('Error al cargar perfiles') });
    }
    if (this.svc.roles().length === 0) {
      this.svc.cargarRoles().subscribe();
    }
  }

  abrirCrear(): void {
    this.form = { id_perfil: undefined, nombre: '', rol: undefined };
    this.modoEditar.set(false);
    this.modalAbierto.set(true);
  }

  abrirEditar(p: Perfil): void {
    this.form = { ...p };
  ...
  guardar(): void {
    if (!this.form.id_perfil || !this.form.nombre?.trim() || !this.form.rol) {
      this.toast.warn('Completa todos los campos obligatorios.');
      return;
    }
    this.guardando.set(true);
    if (this.modoEditar()) {
      this.svc.actualizarPerfil(this.form.id_perfil!, this.form).subscribe({
        next: () => { this.toast.success('Perfil actualizado.'); this.cerrarModal(); this.guardando.set(false); },
        error: () => { this.toast.error('Error al actualizar.'); this.guardando.set(false); }
      });
    } else {
      this.svc.crearPerfil(this.form).subscribe({
        next: () => { this.toast.success('Perfil creado.'); this.cerrarModal(); this.guardando.set(false); },
        error: (err) => {
          const msg = err?.error?.id_perfil?.[0] || 'Error al crear perfil.';
  ...
  confirmarEliminar(p: Perfil): void { this.eliminarTarget.set(p); }

  eliminar(): void {
    const target = this.eliminarTarget();
    if (!target) return;
    this.guardando.set(true);
    this.svc.eliminarPerfil(target.id_perfil).subscribe({
      next: () => { this.toast.success('Perfil eliminado.'); this.eliminarTarget.set(null); this.guardando.set(false); },
      error: () => { this.toast.error('Error al eliminar. Puede tener usuarios asociados.'); this.guardando.set(false); }
    });
  }
}